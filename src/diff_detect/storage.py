...
def load_selection_challenge(
    ...
    return SelectionChallenge.model_validate_json(
...
def dataset_image_index(dataset_id: str = DEFAULT_DATASET_ID) -> dict[tuple[str, str], Image]:
...
def coerce_image_view(image_spec: ImageView | Image | ImageKey | dict[str, Any]) -> ImageView:
...
def submission_row_from_selection_choice(row: dict[str, Any]) -> dict[str, Any]:
    ...
    selected_image_id = artifacts.get("selected_image_id") or selected_image_id_from_row(row)
    task_id = task_id_for(row["challenge_id"], int(row["task_index"]))
    return {
        "id": row.get("id"),
...
def build_rating_options(
    ...
    options = [
        RatingOption(
            ...
            source="self",
            dataset_id=own_submission.get("dataset_id") or dataset_id or DEFAULT_DATASET_ID,
            task_id=task_id,
    ...
    if peer_submission:
        options.append(
            RatingOption(
                ...
                source="peer",
                dataset_id=peer_submission.get("dataset_id") or dataset_id or DEFAULT_DATASET_ID,
                task_id=task_id,
